          ...
          <li key={index}>
            {option.subOptions ? (
              <div className="group cursor-pointer relative">
                ...
                >
                  {option.subOptions.map((subOption: { title: string; link: string }, index: number) => (
                    <li key={index}>
                        <Link
                        href={subOption?.link}
                ...
                <Link
                  href={option.link}
                  className={`header_menu_shadow text-nowrap  duration-700 px-2 py-3 rounded-lg men hover:shadow-[0px_2px_0px_0px_#FFBF3F] ${border === option.title ? 'shadow-[0px_2px_0px_0px_#FFBF3F] ':'' }` } 
                  onClick={()=> handleBottomBorder(option.title) } 
                >
                  {option.title}
                </Link>
